...
- 初回適用: `nix run .#home-manager -- switch --flake .#kakudo --impure`
- 通常適用: `home-manager switch --flake .#kakudo --impure`
...
- `nixpkgs.url = "github:NixOS/nixpkgs/nixos-26.05";`を設定する。
- `home-manager.url = "github:nix-community/home-manager/release-26.05";`を設定する。
- `home-manager.inputs.nixpkgs.follows = "nixpkgs"`を設定する。
- systemは`aarch64-darwin`に固定する。
- outputは`homeConfigurations."kakudo"`の1つにする。
- `apps.aarch64-darwin.home-manager`を定義し、`flake.lock`で固定されたHome Manager CLIを初回適用に使う。
- `home.username`と`home.homeDirectory`は`builtins.getEnv "USER"`と`builtins.getEnv "HOME"`から取得する。
- `USER`または`HOME`が空の場合は、`--impure`不足としてassertionで失敗させる。

### home/default.nix方針

- 共通moduleをimportする。
- `home.stateVersion = "26.05";`を設定する。
- `programs.home-manager.enable = true`を設定する。
- Home Manager moduleで管理できるGUIアプリは、アプリ本体もHome Managerで管理する。
- フォントは第一段階では管理しない。

`home.stateVersion`は、Home Managerの互換性基準を固定する値である。パッケージやHome Manager inputのバージョン固定ではなく、ファイル配置や既定値の互換性を保つためのものなので、通常の`flake.lock`更新では変更しない。

`home.username`と`home.homeDirectory`を環境変数から取得するため、Home Managerのbuild/switchには`--impure`を付ける。これはユーザー名やホームディレクトリを公開リポジトリへ固定しないための例外であり、local layerの設定ファイルはNix評価時に読まない。
...
`home-manager` コマンドがHome Manager管理下で利用可能になった後は、次の短い形式も使える。

```sh
home-manager build --flake .#kakudo --impure
```
...
- local layer初期化はJSON mergeより先に実行する。
- `json-merge.nix`は、local layer初期化後に動くHome Manager activation DAG entryとして実装する。
...
- `programs.bash.shellOptions`: `checkwinsize`、`histappend`
- `programs.bash.historySize`
- `programs.bash.historyFileSize`
- `programs.bash.historyIgnore`
- `programs.bash.historyControl`
- `programs.bash.shellAliases`: `ll`、`la`
- `programs.bash.initExtra`: git completion/prompt、prompt、local bashrc source
...
`XDG_CONFIG_HOME`を既定値以外にする場合は、Home Manager側で生成するinclude pathも合わせる。Git config内ではshellの`${XDG_CONFIG_HOME:-...}`展開を期待しない。実装では`programs.git.includes`を使い、`config.xdg.configHome`相当の値からlocal Git config pathを組み立てる。
...
local ignore fileは第一段階では作らない。ignore patternは共通化可能な`.DS_Store`のみを`programs.git.ignores`で管理する。
...
Home Manager公式moduleがないため、自前moduleで`~/.config/gdb/gdbinit`を生成する。CLI packageとして`gdb`は`tools.nix`で管理する。
...
- merge前に出力先の親ディレクトリ`~/.claude`を作成する。
...
- buildが成功する。
- Ghosttyの既定packageを含めて`aarch64-darwin`向けにbuildできる。
...
- activation DAG上でlocal layer初期化がJSON mergeより先に実行される。
